...
@app.route('/raid_status')
def raid_status():
    raid_volumes = os.popen('ls /dev/md* | cut -d"/" -f3').read().split()

    status = {volume: get_raid_detail(volume) for volume in raid_volumes if os.path.exists(f'/dev/{volume}')}
    return jsonify(status)

@app.errorhandler(404)
def page_not_found(e):
    return redirect(url_for('raid_status'))

if __name__ == '__main__':
    config = configparser.ConfigParser()
    config.read('raid_endpoint.conf')
